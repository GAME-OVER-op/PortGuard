...
fn binary_candidates(proto: Proto) -> Vec<String> {
    let name = binary_basename(proto);
    [
        format!("/system/bin/{name}"),
        format!("/system/xbin/{name}"),
        format!("/system_ext/bin/{name}"),
...
fn resolve_xtables_binary(proto: Proto) -> String {
    let candidates = binary_candidates(proto);
    ...
    candidates
        .last()
        .cloned()
        .unwrap_or_else(|| binary_basename(proto).to_string())
}
