use crate::{
    ...
    types::{PackageIndex, RuleCounter},
};
...
pub fn process_counters(
    ...
    for counter in counters {
        ...
        if delta_packets > 0 || delta_bytes > 0 {
            ...
            logger::info(
                "detector",
                &format!(
                    "blocked localhost probe: {} ports=[{}] packets+={} bytes+={}",
    ...
    for (uid, events) in &state.by_uid {
        ...
        let last_warn = state.last_warn_epoch.get(uid).copied().unwrap_or(0);
        ...
        if uid_packages.iter().any(|pkg| cfg.package_is_scan_ignored(pkg) || cfg.package_is_trusted(pkg)) {
            continue;
        }
        ...
        logger::warn(
            "detector",
            &format!(
                "possible localhost scan detected: {} unique_ports={} exact_ports={} total_attempts={} rule_hits={} grouped_hits={} window={}s ports=[{}] reaction_mode={}",
        ...
        state.last_warn_epoch.insert(*uid, now);
